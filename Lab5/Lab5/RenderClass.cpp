...
void RenderClass::RenderCubes(XMMATRIX view, XMMATRIX proj) {
    ...
    XMMATRIX modelMat = XMMatrixRotationY(m_CubeAngle);
    XMMATRIX vpMat = view * proj;
    XMMATRIX mT = XMMatrixTranspose(modelMat);
    XMMATRIX vpT = XMMatrixTranspose(vpMat);

    m_pDeviceContext->UpdateSubresource(m_pModelBuffer, 0, nullptr, &mT, 0, 0);

    D3D11_MAPPED_SUBRESOURCE mapRes;
    HRESULT hr = m_pDeviceContext->Map(m_pVPBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapRes);
    if (SUCCEEDED(hr)) {
        memcpy(mapRes.pData, &vpT, sizeof(XMMATRIX));
        m_pDeviceContext->Unmap(m_pVPBuffer, 0);
    }
    ...
    XMMATRIX orbitMat = XMMatrixRotationY(orbit) * XMMatrixTranslation(6.0f, 0.0f, 0.0f) * XMMatrixRotationY(-orbit);
    XMMATRIX orbitT = XMMatrixTranspose(orbitMat);
    m_pDeviceContext->UpdateSubresource(m_pModelBuffer, 0, nullptr, &orbitT, 0, 0);
    m_pDeviceContext->DrawIndexed(36, 0, 0);
}
