...
WCHAR szTitle[MAX_LOADSTRING] = L"Advanced Lab5 Kazakevich";
    ...
    _In_ int nCmdShow)
{
    ...
    // Удаляем имя исполняемого файла
    size_t pos = path.find_last_of(L"\\/");
    if (pos != std::wstring::npos)
        path = path.substr(0, pos); // Теперь путь = ...\x64\Debug
    ...
    // Переходим в подпапку Lab5, где лежат шейдеры
    path += L"\\Lab5";

    // Устанавливаем новую рабочую директорию
